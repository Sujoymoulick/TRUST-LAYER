...
export default function PersonalKeys() {
    ...
    return (
      <div className="max-w-4xl mx-auto space-y-8 pb-20">
        <div>
          ...
          <p className="text-xs font-bold text-gray-500 uppercase mt-1 tracking-widest">Manage your Crifolayer production access.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="brutal-card">
            ...
            <div className="h-40 relative border-b-4 border-l border-slate-200 dark:border-zinc-800 ml-8 mb-4 opacity-30 blur-sm">
            ...
            <div className="flex justify-between font-bold text-[10px] uppercase text-gray-300">
          ...
          <div className="brutal-card flex flex-col justify-center gap-4 bg-brutal-navy text-white">
            ...
            <p className="text-xs font-bold leading-relaxed opacity-80 uppercase">
              Integrate the Crifolayer protocol into your own applications using our high-performance SDK.
            </p>
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-20 relative">
          ...
          <div className="bg-white border-[4px] border-black shadow-lg max-w-sm w-full p-6 flex flex-col gap-6">
            ...
            <div className="flex flex-col sm:flex-row gap-4 mt-2">
              ...
              <button 
                ...
                className="brutal-btn flex-1 bg-red-500 text-white px-4 py-3 border border-slate-200 dark:border-zinc-800 font-bold uppercase text-[10px] tracking-widest shadow-md hover:shadow-md hover:translate-x-[4px] hover:translate-y-[4px] transition-all"
              >
                Yes, Revoke
              </button>
            </div>
      ...
      <div className="brutal-card shadow-lg">
        ...
        {apiKeys.length === 0 ? (
          ...
          <div className="space-y-4">
            {apiKeys.map(key => (
              <div key={key._id} className="border border-slate-200 dark:border-zinc-800 p-4 bg-white flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <span className="font-bold">{key.name}</span>
                    {key.status === 'active' && <span className="bg-brutal-green text-black text-[10px] uppercase font-bold px-2 py-0.5">Active</span>}
                  </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="brutal-card">
          <h3 className="font-display text-lg uppercase mb-6">API Utilization</h3>
          <div className="h-40 relative border-b-4 border-l border-slate-200 dark:border-zinc-800 ml-8 mb-4">
            <svg className="w-full h-full" preserveAspectRatio="none" viewBox="0 0 400 100">
              <path
      ...
      <div className="brutal-card p-0 overflow-hidden shadow-lg">
        <h3 className="font-display text-lg uppercase p-4 border-b border-slate-200 dark:border-zinc-800 bg-gray-50">Recent API Transactions</h3>
          ...
          <table className="data-table">
            ...
            <tbody>
              {records.length > 0 ? (
                records.map((r) => (
                  <tr key={r.id}>
                    <td className="uppercase font-mono text-xs font-bold">POST /verify</td>
                    ...
                    <td>
                      <div className={`w-4 h-4 border border-slate-200 dark:border-zinc-800 mx-auto ${r.verification_status === 'verified' ? 'bg-brutal-green' : 'bg-brutal-pink'}`} />
                    </td>
                  </tr>
                ))
              ) : (
